...
// 부족한 만큼 폴백 목록에서 채워 넣어 항상 fiction 10 + nonfiction 10을 보장한다.
async function selectBooks(llmBooks, zodiac, mbti, env) {
  ...
  function bucketFor(category) {
    return category === 'nonfiction' ? nonfiction : fiction
  }
  ...
  if (fiction.length < 10 || nonfiction.length < 10) {
...
- 소설(장편/단편/시/희곡 등 문학) 열 권 (category: "fiction")
- 비소설(에세이, 자기계발, 인문교양, 과학, 역사 등) 열 권 (category: "nonfiction")
같은 카테고리 안에서도 서로 다른 장르로 골라주세요.

반드시 아래와 같은 JSON 배열 형식으로만 답하세요. 배열 순서는 fiction 열 권, nonfiction 열 권 순으로 정렬하고, 총 20개의 객체를 포함해야 하며, 다른 텍스트는 절대 포함하지 마세요. 아래는 형식 예시입니다 (실제로는 각 category당 10개씩 채워주세요):
[
  {
    "title": "책 제목",
    "author": "저자명",
